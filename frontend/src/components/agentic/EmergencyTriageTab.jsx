  ...
  return (
    <div className="glass-panel p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        <div>
          <label className="text-[11px] text-slate-400 block font-mono">GCS (3-15):</label>
          <input
            type="number"
            value={traumaVitals.gcs}
            onChange={(e) => setTraumaVitals({ ...traumaVitals, gcs: Number(e.target.value) })}
            className="w-full bg-slate-950/70 border border-slate-800 rounded-lg p-2 text-xs text-slate-200"
          />
        </div>
        <div>
          <label className="text-[11px] text-slate-400 block font-mono">Systolic BP:</label>
          <input
            type="number"
        ...
        <div>
          <label className="text-[11px] text-slate-400 block font-mono">Diastolic BP:</label>
          <input
            type="number"
        ...
        <div>
          <label className="text-[11px] text-slate-400 block font-mono">Heart Rate:</label>
          <input
            type="number"
        ...
        <div>
          <label className="text-[11px] text-slate-400 block font-mono">SpO2 (%):</label>
          <input
            type="number"
      ...
      <button
        ...
        disabled={loading}
        className="px-4 py-2 rounded-lg bg-rose-700 hover:bg-rose-600 text-white font-medium text-xs flex items-center gap-2 shadow-sm cursor-pointer disabled:opacity-50 transition-colors"
      >
        ...
        <div className="p-4 rounded-lg bg-slate-950/70 border border-slate-800 space-y-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
            <div className="p-2.5 rounded bg-slate-900 border border-slate-800">
              <span className="text-slate-500 block text-[10px] font-mono">MAP</span>
              <span className="font-bold text-slate-200">{agentOutput.data.calculatedMetrics?.map}</span>
            </div>
            <div className="p-2.5 rounded bg-slate-900 border border-slate-800">
              <span className="text-slate-500 block text-[10px] font-mono">Shock Index</span>
              <span className="font-bold text-rose-400">{agentOutput.data.calculatedMetrics?.shockIndex}</span>
            </div>
            <div className="p-2.5 rounded bg-slate-900 border border-slate-800">
              <span className="text-slate-500 block text-[10px] font-mono">Emergency Token</span>
              <span className="font-bold text-slate-200">{agentOutput.data.breakGlassToken?.tokenId}</span>
            </div>
            <div className="p-2.5 rounded bg-slate-900 border border-slate-800">
              <span className="text-slate-500 block text-[10px] font-mono">Valid Duration</span>
